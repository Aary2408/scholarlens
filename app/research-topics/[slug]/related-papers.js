...
export default function RelatedPapers({ query }) {
  ...
  return <div className="space-y-3">{papers.map((paper) => <article key={paper.id} className="rounded-lg border border-border bg-card p-5"><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>{paper.year || "Year unknown"}</span><span className="text-border">•</span><span>{paper.venue}</span></div><Link prefetch={false} href={`/paper/${encodeURIComponent(paper.id)}`} className="mt-2 block text-base font-semibold leading-6 hover:text-primary">{paper.title}</Link><p className="mt-2 text-sm text-muted-foreground">{(paper.authors || []).slice(0, 3).join(", ") || "Author information unavailable"}{paper.authors?.length > 3 ? " et al." : ""}</p><Link prefetch={false} href={`/paper/${encodeURIComponent(paper.id)}`} className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary">Read paper <ArrowUpRight className="h-3.5 w-3.5" /></Link></article>)}</div>;
}
